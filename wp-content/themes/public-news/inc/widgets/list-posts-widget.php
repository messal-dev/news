<?php
if ( ! class_exists( 'Collective_News_List_Posts_Widget' ) ) {
	/**
	 * Adds Collective News List Posts Widget.
	 */
	class Collective_News_List_Posts_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$list_posts_widget = array(
				'classname'   => 'widget adore-widget list-widget style-2',
				'description' => __( 'Retrive List Posts Widgets', 'public-news' ),
			);
			parent::__construct(
				'collective_news_list_posts_widget',
				__( 'Collective News: List Posts Widget', 'public-news' ),
				$list_posts_widget
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$read_more_button_label = ( ! empty( $instance['read_more_button_label'] ) ) ? $instance['read_more_button_label'] : __( 'Read More', 'public-news' );
			$list_posts_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$list_posts_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			$viewall_button_link    = ( ! empty( $instance['view_all_button_link'] ) ) ? $instance['view_all_button_link'] : esc_url( get_category_link( $list_posts_category ) );

			echo $args['before_widget'];
			?>
				<div class="widget-header">
				<?php
				if ( ! empty( $list_posts_title ) ) {
				...
				<div class="adore-widget-body">
					<div class="post-list-widget-wrapper">
						...
						$list_posts_widgets_args = array(
							...
							'offset'         => absint( $list_posts_post_offset ),
							'orderby'        => 'date',
							'order'          => 'desc',
		...
		public function form( $instance ) {
			...
			$read_more_button_label = isset( $instance['read_more_button_label'] ) ? $instance['read_more_button_label'] : __( 'Read More', 'public-news' );
			$list_posts_post_offset = isset( $instance['offset'] ) ? absint( $instance['offset'] ) : '';
			$list_posts_category    = isset( $instance['category'] ) ? absint( $instance['category'] ) : '';
			?>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Section Title:', 'public-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'view_all_button_label' ) ); ?>"><?php esc_html_e( 'View All Button:', 'public-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'view_all_button_link' ) ); ?>"><?php esc_html_e( 'View All Button URL:', 'public-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'read_more_button_label' ) ); ?>"><?php esc_html_e( 'Read More Button Label:', 'public-news' ); ?></label>
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php esc_html_e( 'Number of posts to displace or pass over:', 'public-news' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'offset' ) ); ?>" type="number" step="1" min="0" value="<?php echo absint( $list_posts_post_offset ); ?>" size="3" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'category' ) ); ?>"><?php esc_html_e( 'Select the category to show posts:', 'public-news' ); ?></label>
