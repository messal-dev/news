<?php
if ( ! class_exists( 'Collective_News_Social_Widget' ) ) {
	/**
	 * Adds Collective News Social Widget.
	 */
	class Collective_News_Social_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$collective_news_social_widget = array(
				'classname'   => 'widget adore-widget social-widget style-1',
				'description' => __( 'Retrive Social Widget', 'collective-news' ),
			);
			parent::__construct(
				'collective_news_social_widget',
				__( 'Collective News: Social Widget', 'collective-news' ),
				$collective_news_social_widget
			);
		}
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>"><?php esc_html_e( 'Open Social link in New Tab', 'collective-news' ); ?>:</label>
				<input type="checkbox" id="<?php echo esc_attr( $this->get_field_id( 'open_link_new_tab' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_link_new_tab' ), 'collective-news' ); ?>"  <?php checked( $open_link_new_tab, true ); ?> />
			</p>
			<?php
			for ( $i = 1; $i <= 4; $i++ ) {
				...
				?>
				<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'link' . '-' . $i ) ); ?>"><?php echo sprintf( esc_html__( 'Link %d :', 'collective-news' ), $i ); ?></label>
		...
		public function update( $new_instance, $old_instance ) {
			...
			$instance['open_link_new_tab'] = collective_news_sanitize_checkbox( $new_instance['open_link_new_tab'] );
			for ( $i = 1; $i <= 4; $i++ ) {
